...
<style>
    ...
    .stMetric { background: #1e2130; border-radius: 10px; padding: 8px; }
    h1 { color: #4fc3f7; }
    h2 { color: #81d4fa; }
    .pred-box {
...
with st.sidebar:
    st.image(
        ...
        "4/46/Universidad_San_Sebasti%C3%A1n_Logo.svg/320px-"
        "Universidad_San_Sebasti%C3%A1n_Logo.svg.png",
        width=200,
    )
    st.markdown("## 🧬 Solemne 2 — MDSS")
    st.markdown("**Magíster en Data Science**  \nUniversidad San Sebastián")
    st.divider()
    page = st.radio(
        "Navegar",
...
if page == "📊 Dashboard del Modelo":
    ...
    st.markdown(
        ...
        "sin Peso ni Altura (Solemne 1, USS MDSS 2025)"
    )
    ...
    st.dataframe(
        rd.style.applymap(color_f1, subset=["F1-Score"]),
        use_container_width=True
    )

    # ── Notas analíticas ──────────────────────
    st.divider()
    st.subheader("💡 Hallazgos clave (Solemne 1)")
